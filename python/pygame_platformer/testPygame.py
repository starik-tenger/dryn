...
def draw():
    ...
    for x in range(map.lightMap.startPoint.x-1, map.lightMap.endPoint.x+1):
            for y in range(map.lightMap.startPoint.y-1, map.lightMap.endPoint.y+1):
                if map.blocks[y*map.sizeX+x].type == 1:
                    image_block = setAlpha(image_block, int(map.lightMap.blocks[y*map.sizeX+x].light*255/map.lightMap.power), None)
                ...
                #pygame.draw.rect(screen, (0,0,0), ((x*blockSize-cam.x)*cam.scale, (y*blockSize-cam.y)*cam.scale, blockSize*cam.scale, blockSize*cam.scale))
    for door in map.doors:
        door.draw()
    ...
    for x in range(map.lightMap.startPoint.x-1, map.lightMap.endPoint.x+1):
        for y in range(map.lightMap.startPoint.y-1, map.lightMap.endPoint.y+1):
                ...
                block.fill((0,0,0,int(255-map.lightMap.blocks[y*map.sizeX+x].light*255/map.lightMap.power)))
